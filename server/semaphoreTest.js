...
async function sendSemaphoreMessage(message) {
  ...
  const MOBILE_NUMBER = "09965954484"; 
...
async function handleTelemetryData(telemetryData) {
  ...
  if (telemetryData.engineStatus === "On" || telemetryData.engineStatus === "Off") {
    const formattedTimestamp = convertTimestampToDate(telemetryData.timestamp);
    const message = `Ignition ${telemetryData.engineStatus} for Vehicle ${telemetryData.deviceID} at ${formattedTimestamp}.`;
    await sendSemaphoreMessage(message);
  }
